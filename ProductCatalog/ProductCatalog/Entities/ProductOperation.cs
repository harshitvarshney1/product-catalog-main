    ...
    public class ProductOperation : ProductStore
    {
        ...
        public static void AddProduct(string ProductName, string shortCode, string desc, int price, string manufactureName)
        {
                ...
                do
                {
                    ...
                    var data = CategoryOperation.categories.Single((a) => a.ID == id);
                    if (data != null)
        ...
        {
            var categoryToRemove = products.Single(r => r.ShortCode == shortcode);
        ...
        public static void SearchByID(int Id)
        {
            ...
            if (data.Count > 0)
            {
                data.ForEach((i) =>
                {
                    Console.WriteLine($"{i.Product_ID} \t\t {i.Name}\t\t{i.ShortCode}\t\t{i.Description}\t\t{i.SellingPrice}");
                });
            }
            else
            {
        ...
        {
            var data = products.FindAll((i) => i.Name == name);
            if (data.Count > 0)
            {
                data.ForEach((i) =>
                {
        ...
        {
            var data = products.FindAll((i) => i.ShortCode == shortCode);
            if (data.Count > 0)
            {
                data.ForEach((i) =>
                {
